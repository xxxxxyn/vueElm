      ...
      <span :class="{sideBarTag:true,sideBarSelected:targetIndex===index}"
        ...
        <div class="foodMenuBox" ref="foodMenuBox">
            <div v-for="(category,index) in menuList" ref="foodBoxList">
                <h2>{{category.name}}</h2>
                <!--每一款食物-->
                <OrderMenuFood ref='eachFood' v-for="(food,index) in category.foods" v-bind:foodFromParent="food"
                               v-if="food">
                </OrderMenuFood>
            </div>
            <!--购物车去结算底栏-->
            <div class="currentShopCart" @click="showCart($event)">
                ...
                <button v-if="!notReachMiniAmount" @click="goToPay($event)">去结算</button>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            getMenu: function () {
                let arr
                this.axios
                    .get(window._CONFIG['local'] + 'api/api_shop_menu.json')
                    .then(res => (
