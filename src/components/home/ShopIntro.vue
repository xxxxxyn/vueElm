  ...
  <div >
    <div class="outerBoxShop">
      ...
      <div class="infoBox">
        ...
        <div class="line2">
          <span class="rating"><svg t="1578294589881" class="icon" viewBox="0 0 1024 1024" version="1.1"
        ...
        <div class="rcm" v-if="shop.restaurant.recommend_reasons.length!==0">
          <span v-for="rs in shop.restaurant.recommend_reasons">{{rs.name}}</span>
        </div>
        <!--activities第一个是新用户满减，第二个是满多少减多少，要拆分-->
        <div class="atvt" v-if="shop.restaurant.activities.length!==0" :class="{'isOpenAt':isOpenAt}">
          <div>
            <span>首单减{{Math.abs(shop.restaurant.activities[0].attribute)}}</span>
            <span v-for="at in shop.restaurant.activities[1].description.split('，')">
            {{at}}
          </span>
          </div>
          <span class='openBtn' ref="openBtn" @click="openActivities()">∨</span>
        </div>
      </div>
